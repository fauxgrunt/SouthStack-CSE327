...
type Phase = 'idle' | 'initializing' | 'ready' | 'generating' | 'installing' | 'executing' | 'error';
...
const App: React.FC = () => {
  ...
  const [processStream, setProcessStream] = useState<ReadableStream<string> | null>(null);
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      setIsOnline(false);
      addLog('[CONNECTION] Internet connection lost - switching to offline mode', 'warning');
    };
...
const express = require('express');  // WRONG - express not available offline!`;
      ...
      if (isOffline) {
        ...
        addLog('[OFFLINE MODE] Skipping npm install - using Node.js standard library only', 'warning');
      }
      ...
      if (!isOffline) {
        ...
        if (installExitCode !== 0) {
          addLog('[WARNING] npm install failed, attempting to run anyway...', 'warning');
        } else {
      ...
      const systemMessageStream = new ReadableStream({
        start(controller) {
          if (isOffline) {
            controller.enqueue('\x1b[1;33m[SYSTEM] Offline Mode Detected. Skipping npm installs. Using Node.js Standard Library only.\x1b[0m\r\n');
            controller.enqueue('\x1b[1;36m[SYSTEM] Available modules: http, https, fs, path, crypto, events, util, url, querystring, stream, buffer, os, process\x1b[0m\r\n\r\n');
          }
          controller.enqueue('\x1b[1;32m[SYSTEM] Starting Node.js process...\x1b[0m\r\n\r\n');
          controller.close();
        }
      });
      ...
      try {
        ...
        nodeProcess.exit.then((exitCode) => {
          if (exitCode !== 0) {
            addLog(`[WARNING] Process exited with code ${exitCode}. Check terminal for errors.`, 'warning');
            // If there's a module error, log it for potential self-healing
            addLog('[INFO] If you see "Cannot find module" errors, try rephrasing your prompt to use only Node.js built-in modules.', 'info');
          } else {
        ...
        addLog(`[ERROR] Failed to start process: ${spawnError.message}`, 'error');
        ...
        const errorStream = new ReadableStream({
          start(controller) {
            ...
            controller.enqueue(`\x1b[33m[HINT] The generated code may have syntax errors or use unavailable modules.\x1b[0m\r\n`);
            controller.close();
          }
        });
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4" style={{ fontFamily: "'Fira Code', monospace" }}>
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${
                phase === 'ready' ? 'bg-green-500 heartbeat-pulse' : 
                phase === 'error' ? 'bg-red-500' : 
                'bg-yellow-500 animate-pulse'
              }`} />
              <span className="font-medium flex items-center gap-2">
                {phase === 'idle' && 'Not Initialized'}
                {phase === 'initializing' && (<>Initializing... <svg className="spinner w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></>)}
                {phase === 'ready' && '[READY] Offline'}
                {phase === 'generating' && (<>Generating Code... <svg className="spinner w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></>)}
                {phase === 'installing' && (<>Installing Dependencies... <svg className="spinner w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></>)}
                {phase === 'executing' && (<>Executing Code... <svg className="spinner w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></>)}
                {phase === 'error' && '[ERROR]'}
              </span>
            </div>
        ...
        {phase === 'initializing' && initProgress && (
          <div className="bg-slate-900/50 backdrop-blur-md rounded-lg p-4 mb-6 border border-slate-700 shadow-xl">
            <div className="text-sm text-blue-400 mb-2" style={{ fontFamily: "'Fira Code', monospace" }}>Loading Model...</div>
            <div className="text-xs text-gray-400" style={{ fontFamily: "'Fira Code', monospace" }}>{initProgress}</div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            {!isOnline && phase === 'ready' && (
              <div className="bg-orange-900/20 backdrop-blur-md border border-orange-700 rounded-lg p-4 shadow-xl">
                <h3 className="text-orange-400 font-semibold mb-2" style={{ fontFamily: "'JetBrains Mono', monospace" }}>[OFFLINE MODE ACTIVE]</h3>
                <p className="text-orange-200 text-sm mb-2" style={{ fontFamily: "'Fira Code', monospace" }}>
                  Internet connection not detected. The AI has been configured to generate code using ONLY Node.js built-in modules.
                </p>
                <p className="text-orange-300 text-xs" style={{ fontFamily: "'Fira Code', monospace" }}>
                  Available: http, https, fs, path, crypto, events, util, url, querystring, stream, buffer, os, process
                </p>
                <p className="text-orange-300 text-xs mt-2" style={{ fontFamily: "'Fira Code', monospace" }}>
            ...
            <div className="bg-slate-900/50 backdrop-blur-md rounded-lg border border-slate-700 shadow-xl overflow-hidden">
              ...
              <div className="p-6">
                <div className="bg-slate-950/70 rounded-lg p-4 max-h-[400px] overflow-y-auto text-sm border border-slate-700" style={{ fontFamily: "'Fira Code', monospace" }}>
                  {logs.length === 0 ? (
                    <p className="text-gray-500 italic">No logs yet. Click Initialize to begin.</p>
                  ) : (
                    logs.map((log, idx) => (
                      <div
                        key={idx}
          ...
          <div className="bg-slate-900/50 backdrop-blur-md rounded-lg p-6 border border-slate-700 shadow-xl">
            <h2 className="text-xl font-semibold mb-4" style={{ fontFamily: "'JetBrains Mono', monospace" }}>Terminal Output</h2>
            <Terminal processStream={processStream} height="calc(100vh - 250px)" />
          </div>
        </div>
      </div>
    </div>
