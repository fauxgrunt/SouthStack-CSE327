  ...
  return (
    <div className="relative">
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs transition hover:border-zinc-500 hover:bg-zinc-800 ${statusMeta.badge} ${statusMeta.text}`}
      >
      ...
      {isOpen && (
        <div className="absolute right-0 top-[calc(100%+10px)] z-50 w-[340px] rounded-2xl border border-zinc-700 bg-zinc-900/95 p-4 shadow-2xl backdrop-blur">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-md border border-zinc-700 px-2 py-1 text-[10px] text-zinc-400 transition hover:text-zinc-200"
            >
              Close
            </button>
          </div>
          ...
          <div className="mb-3">
            ...
            <input
              ...
              className="h-9 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 text-sm text-zinc-100 outline-none placeholder:text-zinc-500 focus:border-cyan-400/50"
            />
          </div>
          ...
          <div className="mb-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`inline-flex items-center justify-center gap-1 rounded-lg border px-3 py-2 text-xs transition ${
                  role === "master"
          ...
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-cyan-500/40 bg-cyan-500/15 px-3 py-2 text-xs font-semibold text-cyan-100 transition hover:bg-cyan-500/25 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isConnecting ? (
          ...
          <div className="mt-4 rounded-lg border border-zinc-700 bg-zinc-950/60 p-2">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-[11px] text-zinc-400">Connected Peers</span>
              <button
                type="button"
                onClick={onDisconnectAll}
                disabled={connectedPeers.length === 0}
                className="rounded-md border border-zinc-700 px-2 py-0.5 text-[10px] text-zinc-300 transition hover:bg-zinc-800 disabled:opacity-40"
              >
                Disconnect All
              </button>
            </div>
            ...
            {openPeers.length > 0 && (
              <p className="mt-2 text-[10px] text-zinc-500">
                {openPeers.length} live peer{openPeers.length > 1 ? "s" : ""} in
                room.
              </p>
            )}
          </div>
